...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/statuspage-v55.css?v=5.5.0">
</head>
<body>
<header class="sp-top"><div class="sp-wrap sp-top-inner"><a class="sp-brand" href="/"><img src="/assets/img/fare-brothers-logo.png" alt="Fare Brothers logo"><span><strong>Fare Brothers</strong><small><?= e($statusPageLabel) ?></small></span></a><nav class="sp-nav"><a href="https://farebros.com">Main Website</a><?php if(!empty($payload['groups'])): ?><a href="#groups">Groups</a><?php endif; ?><a href="#systems">Systems</a><a href="#websites">Websites</a><a href="/history.php">History</a><?php if(get_setting('public_subscriptions_enabled','1')==='1'): ?><a href="/subscribe.php">Subscribe</a><?php endif; ?><?php if ($showAdminLink): ?><a class="sp-admin" href="/admin/login.php">Admin</a><?php endif; ?></nav></div></header>
...
<main class="sp-wrap">
    ...
    <section class="sp-section" id="groups" <?= empty($payload['groups']) ? 'hidden' : '' ?>><div class="sp-section-head"><div><h2>Service Groups</h2><p>Related systems organized by business or platform function.</p></div><span class="sp-section-tag" id="statusGroupCount"><?= count($payload['groups']) ?> group<?= count($payload['groups'])===1?'':'s' ?></span></div><div class="v55-public-group-grid" id="statusGroupGrid"><?php foreach($payload['groups'] as $group): $gm=$group['status_meta']; ?><article class="v55-public-group <?= e($gm['class']) ?>"><div class="v55-public-group-head"><div><h3><?= e($group['group_name']) ?></h3><p><?= e($group['description']) ?></p></div><span><?= e($gm['label']) ?></span></div><div class="v55-group-members-public"><?php foreach($group['members'] as $member): ?><div class="v55-group-member"><span class="sp-dot <?= e($member['status_meta']['class']) ?>"></span><div><strong><?= e($member['name']) ?></strong><small><?= e($member['status_meta']['label']) ?><?= !empty($member['dependency_affected'])?' · upstream dependency':'' ?></small></div></div><?php endforeach; ?></div></article><?php endforeach; ?></div></section>

    <section class="sp-section" id="systems"><div class="sp-section-head"><div><h2>Systems</h2><p>Core services and platform components monitored by the status center.</p></div><span class="sp-section-tag">Live status</span></div><div class="sp-list" id="serviceGrid"><?php foreach ($payload['services'] as $service): $overlay = item_schedule_overlay('service', (int)$service['id']); ?><article class="sp-row <?= e($service['class']) ?>"><div class="sp-row-main"><span class="sp-dot"></span><div><strong><?= e($service['name']) ?></strong><p><?= e($service['description']) ?></p><?php if ($overlay): ?><span class="sp-maint-badge <?= e($overlay['phase']) ?>"><?= e(overlay_label($overlay)) ?></span><?php endif; ?><?php if(!empty($service['dependency_affected'])): ?><span class="v55-dependency-badge">Upstream dependency</span><?php endif; ?></div></div><div class="sp-row-status"><strong><?= e($service['label']) ?></strong><small>Code <?= e((string)$service['code']) ?></small></div></article><?php endforeach; ?></div></section>
        ...
        <div class="sp-list" id="websiteTable">
            <?php foreach ($payload['websites'] as $website): $overlay = item_schedule_overlay('website', (int)$website['id']); $points = public_sparkline_points($website['response_series'] ?? []); ?>
                <article class="sp-row sp-row-monitor <?= e($website['class']) ?>">
                    <div class="sp-row-monitor-top"><div class="sp-row-main"><span class="sp-dot"></span><div><strong><?= e($website['name']) ?><?= $website['is_primary'] ? ' · Primary' : '' ?></strong><p><?= e($website['url']) ?></p><?php if ($overlay): ?><span class="sp-maint-badge <?= e($overlay['phase']) ?>"><?= e(overlay_label($overlay)) ?></span><?php endif; ?><?php if(!empty($website['dependency_affected'])): ?><span class="v55-dependency-badge">Upstream dependency</span><?php endif; ?></div></div><div class="sp-row-status"><strong><?= e($website['label']) ?></strong><small><?= $website['last_checked_at'] ? 'Checked ' . e(format_dt($website['last_checked_at'])) : 'Waiting for first check' ?></small></div></div>
...
<script src="/assets/js/statuspage-v55.js?v=5.5.0"></script>
</body>
</html>
