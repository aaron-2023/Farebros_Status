...
function renderServiceRow(item,data){
    ...
    return `<article class="sp-row ${escapeHtml(item.class)}"><div class="sp-row-main"><span class="sp-dot"></span><div><strong>${escapeHtml(item.name)}</strong><p>${escapeHtml(item.description)}</p>${overlay}</div></div><div class="sp-row-status"><strong>${escapeHtml(item.label)}</strong><small>Code ${escapeHtml(item.code)}</small></div></article>`;
}
...
function renderWebsiteRow(item,data){
    ...
    return `<article class="sp-row sp-row-monitor ${escapeHtml(item.class)}">
        <div class="sp-row-monitor-top"><div class="sp-row-main"><span class="sp-dot"></span><div><strong>${escapeHtml(item.name)}${item.is_primary?' · Primary':''}</strong><p>${escapeHtml(item.url)}</p>${overlay}</div></div><div class="sp-row-status"><strong>${escapeHtml(item.label)}</strong><small>${checked}</small></div></div>
